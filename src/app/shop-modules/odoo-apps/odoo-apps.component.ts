...
export class OdooAppsComponent implements OnInit {
  ...
  lastString: any = 'selected categories or version';
  currentCompany: any = '';
  myValue: any = '';
  environment: any = '';
  ...
  setCategoriesHighLight() {
    ...
    if (this.categoriesPasses == "shop/magento-apps/category/magento-sales-management-34") {
      this.currentCompany = 'Sales Management'
    }
    if (this.categoriesPasses == "shop/magento-apps/category/magento-tools-33") {
      this.currentCompany = 'Tools'
    }
  }
  ...
  fiterVersion(event: any) {
    ...
    if(this.versionFitervalue.length==this.versions.length){
      this.lastString="Selected version"
    }
  ...
  if(item){
    ...
    if(str==null){
      str=this.router.url.substring(1);
    }
    this.service.get3(str).subscribe(res => {
      this.isLoadingBool_second = false
      this.isLoadingBool = false;
  ...
  addTowishlist(item: any) {
    ...
    this.service.getAllData('shop/wishlist/add?product_id=' + item.product_var_id).subscribe(result => {
      ...
      if (result.product_added == true) {
        this.callMethod('');
        item.in_wish = true;
        this.service.success('', 'Product added in wishlist', ToasterPosition.bottomLeft);
